...
const boardView = ( ) => {
	...
	fetch( `/tj2024_b_web1/day05/board/view?bno=${ bno }` , option )
		...
		.then( data => {
			...
			boardInfo = data; // - 다른 함수에서 게시물 정보를 이용하기 위해 게시물 정보를 전역변수로 이동
		})
...
const boardDelete = () => {
	...
	fetch(`/tj2024_b_web1/day05/board?bno=${ bno }` , opt)
		.then(response => response.json())
		.then(data => {
			if(data == true){
				alert('삭제 성공');
