...
const findByBno = ( ) => {
	...
	fetch( `/tj2024_b_web1/board/view?bno=${ bno }` , option ) 
		.then( r => r.json() )
		.then( data => {
			console.log( data );
...
const onRplyWrite = () => {
	...
	fetch( '/tj2024_b_web1/board/reply' , option )
		.then( r => r.json() )
		.then( data => {
			if( data == true ){
				alert('댓글쓰기 성공');
...
const replyFindAll = ( ) => {
	...
	const bno = new URL( location.href ).searchParams.get("bno");
	// fetch queryString 
	fetch( `/tj2024_b_web1/board/reply?bno=${bno}` )
		.then( r => r.json() )
		.then( data => {
			console.log( data );
			...
			data.forEach( reply =>{
				html +=`<div class="card mt-3">
						  <div class="card-header">
						  	<img src="/tj2024_b_web1/upload/${ reply.mimg}" style="width:30px;" />
						    ${ reply.mid }
						  </div>
						  <div class="card-body">
						     ${ reply.rcontent }
