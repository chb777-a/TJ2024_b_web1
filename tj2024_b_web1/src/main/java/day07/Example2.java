...
public class Example2 {
	public static void main(String[] args) {
		// 1. ArrayList 인스턴스 생성
			// 1. 인스턴스 : new ArrayList<String>();		// 클래스 타입
			// 2. 변수명 : list , 							// 아무거나
		...
		list.add(2 , "김희철");	// 3번째 위치한 자료를 "김희철" 자료로 변경
		
			// (2) .set(인덱스 , 자료 ) : 리스트 내 지정한 인덱스에 지정한 자료의 요소 수정/변경
		list.set(2, "서장훈");	// 3번째 위치한 자료를 "서장훈" 자료로 변경
		...
		System.out.println(list.size());	// 5 ( 5개 요소 존재 )
		...
		System.out.println(result2);	// -1 : 존재하지 않아서 -1 반환
		
			// (7) .remove( 인덱스/자료 ) : 리스트 내 지정한 인덱스/자료의 요소 삭제
		list.remove(0);	// 0번 인덱스 삭제
		System.out.println(list);	//[강호동, 서장훈, 신동엽, 하하] => 유재석 삭제됨
		
			// (8) .clear() : 리스트 내 모든 요소를 삭제
		
			// (9) .isEmpty() : 리스트 내 요소가 비어있으면 true 1개 이상 존재하면 false 반환
		System.out.println(list.isEmpty()); // false
		
			// (10) 리스트 내 요소들을 순회(하나씩 꺼내기) 하는 방법
				// 1. 일반 for 문
