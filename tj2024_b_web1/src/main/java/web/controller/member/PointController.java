...
public class PointController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// * 동일한 HTTP 메소드 안에서 서로 다른 서비스 구분하는 방법
			// [1] '포인트 로그 내역 전체 조회' 와 '현재 남은 포인트 조회' 를 쿼리스트링 이용하여 서비스 구분
			// type : 조회 서비스 방법 식별 , 1 : '포인트 로그 내역 전체 조회' , 2 : '현재 남은 포인트 조회'
			// '포인트 로그 내역 전체 조회' : localhost:8080/point?type=all	// all : 포인트 로그 내역 전체 조회
			// '포인트 로그 내역 전체 조회' : localhost:8080/point?type=current // current : 현재 남은 포인트 조회
